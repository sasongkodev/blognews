            ...
            <div className="container mx-auto px-4">
                <div className="flex flex-col items-center justify-center gap-6 md:gap-8 text-center">
                    ...
                    <div className="space-y-3 md:space-y-4">
                        ...
                        <p className="max-w-sm md:max-w-md text-slate-400 leading-relaxed mx-auto text-xs md:text-sm px-4">
                    ...
                    <nav className="flex flex-wrap justify-center gap-4 md:gap-6 lg:gap-8">
                        {FOOTER_NAV.map((item) => (
                            <Link
                                key={item.label}
                                href={item.href}
                                className="text-xs md:text-sm font-medium text-slate-400 hover:text-cyan-400 transition-colors"
                            >
                                {item.label}
                            </Link>
                        ))}
                    ...
                    <div className="flex gap-2 md:gap-3">
                        {SOCIAL_LINKS.map(({ icon: Icon, href, label }) => (
                            <a
                                ...
                                className="p-2 md:p-2.5 text-slate-400 hover:text-cyan-400 hover:bg-slate-800 rounded-full transition-all border border-slate-700/50 hover:border-cyan-400/50"
                                aria-label={label}
                            >
                    ...
                    {/* Copyright */}
                    <div className="mt-2 md:mt-4 pt-6 md:pt-8 w-full border-t border-slate-800/50 flex flex-col sm:flex-row items-center justify-center gap-2 text-[10px] md:text-xs text-slate-500">
